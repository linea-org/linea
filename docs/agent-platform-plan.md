...
| ------------------------ | --------------------------------------- |
| `triggerWorkflow`        | `POST /triggers/:slug`                  |
| `getExecution`           | `GET /executions/:id`                   |
| `listWorkflowExecutions` | `GET /workflows/:workflowId/executions` |
| `listExecutions`         | `GET /executions`                       |
| `countNewExecutions`     | `GET /executions/new-count`             |
| `listSignals`            | `GET /signals`                          |
| `getSignalsTrend`        | `GET /signals/trend`                    |
| `getSignal`              | `GET /signals/:id`                      |
| `resolveSignal`          | `POST /signals/:id/resolve`             |
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- | ------------------ |
| System                     | `GET /`, `GET /health`                                                                                                                                                                                                                                                           | Operations                         | None               |
| Authentication             | `/api/auth/*`                                                                                                                                                                                                                                                                    | First-party web and mobile         | Better Auth client |
| Current member             | `GET /me`                                                                                                                                                                                                                                                                        | Operator control plane             | None               |
| API keys                   | `POST /api-keys`, `GET /api-keys`, `DELETE /api-keys/:id`                                                                                                                                                                                                                        | Operator control plane             | None               |
| Workspace approvals        | `GET /approvals`, `POST /approvals/:id/respond`                                                                                                                                                                                                                                  | Operator web and mobile            | None               |
| Production message ingress | `POST /workflows/:workflowId/conversations/:conversationId/messages`                                                                                                                                                                                                             | Trusted backend, transitional      | None               |
| Conversation analysis      | `GET /workflows/:workflowId/conversations/:conversationId/analysis`                                                                                                                                                                                                              | Operator control plane             | None               |
| Workflow execution         | `POST /workflows/:workflowId/executions`, `GET /workflows/:workflowId/executions`                                                                                                                                                                                                | Operator control plane             | List only          |
| Builder testing            | `POST /workflows/:workflowId/test-run`, `POST /workflows/:workflowId/chat-preview`, `GET /workflows/:workflowId/chat-preview/conversations`, `GET /workflows/:workflowId/chat-preview/:conversationId/messages`                                                                  | Builder-only control plane         | None               |
| Workspace executions       | `GET /executions`, `GET /executions/new-count`, `GET /executions/:id`, `POST /executions/:id/steps/:stepId/replay`                                                                                                                                                               | Operator control plane             | All except replay  |
| Notifications              | `GET /notifications`, `GET /notifications/unread-count`, `POST /notifications/:id/read`, `POST /notifications/:id/unread`, `POST /notifications/read-all`, `POST /notifications/:id/archive`, `POST /notifications/:id/unarchive`, `DELETE /notifications/:id`                   | First-party web and mobile         | None               |
| Regression Cases           | `GET /workflows/:workflowId/regression-cases`, `POST /workflows/:workflowId/regression-cases/from-step`, `POST /workflows/:workflowId/regression-cases/from-flag`, `POST /workflows/:workflowId/regression-cases/:id/archive`                                                    | Operator control plane             | None               |
| Regression Runs            | `GET /workflows/:workflowId/regression-runs`, `POST /workflows/:workflowId/regression-runs`, `GET /workflows/:workflowId/regression-runs/:id`                                                                                                                                    | Operator control plane             | None               |
| Secrets                    | `GET /secrets`, `GET /secrets/providers`, `PUT /secrets/:key`, `DELETE /secrets/:key`                                                                                                                                                                                            | Operator control plane             | None               |
| Signals                    | `GET /signals`, `GET /signals/trend`, `GET /signals/:id`, `POST /signals/:id/resolve`                                                                                                                                                                                            | Operator and workspace automation  | Complete           |
| Trigger                    | `POST /triggers/:slug`                                                                                                                                                                                                                                                           | Workspace automation, transitional | `triggerWorkflow`  |
| Workflows                  | `POST /workflows`, `GET /workflows`, `GET /workflows/:id`, `PATCH /workflows/:id`, `PUT /workflows/:id/draft`, `POST /workflows/:id/realtime-token`, `POST /workflows/:id/versions`, `GET /workflows/:id/versions/:versionId`, `POST /workflows/:id/versions/:versionId/publish` | Operator control plane             | None               |
...
The real controller also obtains the authenticated Application principal and idempotency key. They are omitted from this narrow type-flow example. The important rule is that incoming and outgoing data cross the same protocol schemas used by the SDK.
...
These operations are planned, not currently implemented.
