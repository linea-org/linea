...
async function withOrg(fn: (workspaceId: string) => Promise<void>) {
  ...
  const [organization] = await db
    ...
    .values({
      name: 'Eval Cases Service Test Org',
      slug: `eval-cases-service-${suffix}`,
      createdAt: new Date(),
    })
    .returning()
    ...
    try {
      await withOrg(async (workspaceId) => {
        ...
        const workflow = await repositories.workflow.createWorkflow(db, {
          workspaceId,
          name: 'Eval Cases Test Workflow',
          slug: `eval-cases-test-${suffix}`,
        })
        const otherWorkflow = await repositories.workflow.createWorkflow(db, {
          workspaceId,
          name: 'Eval Cases Test Other Workflow',
          slug: `eval-cases-test-other-${suffix}`,
        })
        const version = await repositories.workflow.createWorkflowVersion(db, {
          workflowId: workflow.id,
          graph: { nodes: [], edges: [] },
          contentHash: `eval-cases-test-hash-${suffix}`,
        })
  ...
  it('creates a case from a flag, rejecting a flag with no conversation to snapshot and a flag from another workflow', async () => {
    ...
    try {
      await withOrg(async (workspaceId) => {
        ...
        const workflow = await repositories.workflow.createWorkflow(db, {
          workspaceId,
          name: 'Eval Cases Flag Test Workflow',
          slug: `eval-cases-flag-test-${suffix}`,
        })
        const otherWorkflow = await repositories.workflow.createWorkflow(db, {
          workspaceId,
          name: 'Eval Cases Flag Test Other Workflow',
          slug: `eval-cases-flag-test-other-${suffix}`,
        })
