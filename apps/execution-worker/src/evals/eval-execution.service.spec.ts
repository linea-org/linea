...
async function setUpWorkflow(graph: WorkflowGraph) {
  ...
  const [organization] = await db
    ...
    .values({
      name: "Eval Execution Test Org",
      slug: `eval-execution-test-${suffix}`,
      createdAt: new Date(),
    })
    .returning()
  const workflow = await repositories.workflow.createWorkflow(db, {
    workspaceId: organization.id,
    name: "Eval Execution Test Workflow",
    slug: `eval-execution-workflow-${suffix}`,
  })
  const version = await repositories.workflow.createWorkflowVersion(db, {
    workflowId: workflow.id,
    graph,
    contentHash: `eval-execution-hash-${suffix}`,
  })
  return { organization, workflow, version }
}

describe("EvalExecutionService.runEvalsForVersion", () => {
  ...
  it("aggregates passed/failed across multiple cases in one run", async () => {
    ...
    try {
      ...
      const archived = await repositories.evalCase.createEvalCase(db, {
        workspaceId: organization.id,
        workflowId: workflow.id,
        caseType: "node",
        nodeId: "transform-1",
      ...
      await repositories.evalCase.archiveEvalCase(
        db,
        organization.id,
        archived.id
      )
